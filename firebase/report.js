...
function initPage(){
    firebase.database().ref("analysis/" + userData.currentFarm).orderByChild("availability").equalTo("report").once("value").then(function(snapshot){
...
document.getElementById("send").addEventListener("click", function(){
    if(this.getAttribute("data-key").length>0){
        ...
        messageRef.update({issue:document.getElementById("messageContent").value, issueTime:d.getTime()}).then(()=>{
